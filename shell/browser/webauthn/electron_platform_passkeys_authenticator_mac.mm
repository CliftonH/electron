...
// Status reported when a request or response is malformed or unrepresentable.
// device::MakeCredentialStatus::kAuthenticatorResponseInvalid would be more
// precise, but the request handlers ignore that status coming from a
// kICloudKeychain-type authenticator (see HandleResponse in
// device/fido/make_credential_request_handler.cc and
// device/fido/get_assertion_request_handler.cc), which would leave the page's
// request pending until the WebAuthn timeout. Electron has no request UI the
// user could cancel from, so every failure this authenticator reports must be
// a terminal status; kUserConsentDenied completes the request promptly and
// surfaces to the page as NotAllowedError.
...
// Logged when a cross-origin (iframe) ceremony is rejected: the failure is
// deterministic, but the page only sees a bare NotAllowedError.
...
ElectronPlatformPasskeysAuthenticator::GetAlgorithms() {
  // Advertising exactly ES256 (see the header) makes the request handler
  // pre-filter this authenticator before dispatch. Returning
  // kNoCommonAlgorithms from MakeCredential instead would route into the base
  // GetTouch() no-op and hang the request; only Apple's SPI path can forward
  // the RP's full list.
