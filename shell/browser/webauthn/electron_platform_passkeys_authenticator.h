...
class ElectronPlatformPasskeysAuthenticator : public device::FidoAuthenticator {
 public:
  ...
  // ES256 only: the public ASAuthorization registration API cannot honor other
  // pubKeyCredParams, so the request handler must pre-filter this authenticator
  // when the relying party doesn't accept ES256.
 ...
 private:
  ...
  // Emits a DevTools console error when Apple's response fails parsing or
  // validation — without it, a malformed response would be indistinguishable
  // from the user cancelling the sheet.
